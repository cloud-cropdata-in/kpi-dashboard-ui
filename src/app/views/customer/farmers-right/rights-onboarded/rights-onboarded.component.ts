...
declare var $;
@Component({
  selector: 'app-rights-onboarded',
  templateUrl: './rights-onboarded.component.html',
  styleUrls: ['./rights-onboarded.component.scss']
})
export class RightsOnboardedComponent implements OnInit {
  regionalDetails: any;
  ...
  constructor(private getRightsOnboardedDetail: DashboardService) {
    this.getAnalyticsDetails()
  }
  getAnalyticsDetails() {
    this.getRightsOnboardedDetail.getRightsOnboardedDetails().subscribe(
      (data) => {
        if(data != '' || data != undefined){
          ...
          console.log(this.regionalAchieved +'/'+ this.regionalTarget +'*'+ 100 +" = "+ (this.regionalAchieved / this.regionalTarget) * 100);
          console.log(this.commodityAchieved +'/'+ this.commodityTarget +'*'+100 +" = "+ (this.commodityAchieved / this.commodityTarget) * 100);
          console.log(this.landholdingsAchieved +'/'+ this.landholdingsTarget +'*'+100 +" = "+ (this.landholdingsAchieved / this.landholdingsTarget) * 100);
    ...
    $(document).ready(function () {
      if ($('.grade').find('.dataValue').text()) {
        $('.grade').each(function () {
          ...
          var dataPercentage = Number((dataValue / dataTarget * 100).toFixed(1));
          $(this).find('.dataPercentage').html(dataPercentage + '%');
      ...
      if ($('.regionDetails .data-current-value').is(':empty')) {
        ...
        $('.regionDetails').each(function () {
          ...
          $(this).find('.data-current-value').css('background-color', dataValueColorRange(valueInPercentage))
          $(this).find('.currentValuePercentage').css('border-color', dataValueColorRange(valueInPercentage))
          ...
          if (valueInPercentage > 100) {
            $(this).find('.currentValuePercentage').html(100 + '%');
          } else{
            $(this).find('.currentValuePercentage').html(valueInPercentage + '%');
          }
        });
        ...
        $('.standardScale').each(function () {
          var dataCurrentValue = Number($(this).find('.dataCurrentValue').text());
          var dataTarget = Number($(this).find('.dataTarget').text());
          var valueInPercentage = ((dataCurrentValue / dataTarget) * 100).toFixed(1);
          if(Number(valueInPercentage) > 100){
            $(this).find('.standardScalePercentage').css('background-color', dataValueColorRange(100)).css('width', 100 + '%').html(100 + '%').addClass('text-center');
          }
          else{
          $(this).find('.standardScalePercentage').css('background-color', dataValueColorRange(valueInPercentage)).css('width', valueInPercentage + '%').html(valueInPercentage + '%').addClass('text-center');
          }
        });
